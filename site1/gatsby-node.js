...
exports.createSchemaCustomization = async ({ actions }) => {
  ...
  actions.createFieldExtension({
    name: "videoPassthroughArgs",
    extend(options) {
      const { args } = documentToHtmlString()
      return {
        args,
      }
    },
  ...
  actions.createFieldExtension({
    ...
    extend(options) {
      ...
      return {
        resolve(source) {
          // Assuming 'source' has a 'file' object with a 'url' property
          // Check if 'file' and 'url' exist to avoid errors
          if (source.file && source.file.url) {
            return addURLSchema(source.file.url);
          }
          // Return null or a default value if 'url' is not available
          return null;
        },
      };
    },
  ...
  actions.createTypes(/* GraphQL */ `
    ...
    type ContentfulVideo implements Node & HomepageVideo {
      id: ID!
      url: String @videoUrl
      file: JSON
      title: String!
    ...
    type ContentfulHomepageHero implements Node & HomepageHero & HomepageBlock
      ...
      video: HomepageVideo @link(from: "video_NODE")
    }
